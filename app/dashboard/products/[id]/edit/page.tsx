...
// Parse specs JSON back into editable groups
function parseSpecsToGroups(specs: any): SpecGroup[] {
    if (!specs || typeof specs !== 'object' || Array.isArray(specs)) return [];
    return Object.entries(specs).map(([tabName, entries]: [string, any]) => ({
        tabName,
        entries: entries && typeof entries === 'object' ? Object.entries(entries).map(([key, value]: [string, any]) => ({ key, value: String(value || '') })) : [],
    }));
}
    ...
    useEffect(() => {
        Promise.all([
            ...
            setForm({
                ...
                images: Array.isArray(data.images) ? data.images : [], isPublished: data.isPublished || false,
            ...
            setHighlightSpecs(Array.isArray(data.highlightSpecs) ? data.highlightSpecs : []);
            setSpecGroups(parseSpecsToGroups(data.specs));
            setLoading(false);
        });
    }, [id]);
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className={sectionCls}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label className={labelCls}>RFQ Form</label>
