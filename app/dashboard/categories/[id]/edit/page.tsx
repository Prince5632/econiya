...
export default function EditCategoryPage() {
    ...
    useEffect(() => {
        fetch(`/api/categories/${id}`).then(r => r.json()).then(data => {
            setForm({
                name: data.name || '', slug: data.slug || '', description: data.description || '', image: data.image || '',
                sortOrder: data.sortOrder || 0, isPublished: data.isPublished ?? true,
                metaTitle: data.metaTitle || '', metaDescription: data.metaDescription || '', metaKeywords: data.metaKeywords || '', ogImage: data.ogImage || '',
            }); setLoading(false);
        });
    }, [id]);
    ...
    return (
        <div className="mx-auto max-w-4xl">
            <h1 className="mb-6 text-2xl font-bold text-zinc-900 dark:text-white">Edit Category</h1>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="flex justify-end gap-3">
                    ...
                    <button type="submit" disabled={saving} className="rounded-lg bg-indigo-600 px-6 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50">{saving ? 'Saving...' : 'Update Category'}</button>
                </div>
            </form>
        </div>
    );
