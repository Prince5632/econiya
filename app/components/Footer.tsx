                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
                    ...
                    <div className="space-y-5 lg:col-span-1">
                        ...
                        <div className="flex space-x-3 pt-2">
                            {/* LinkedIn */}
                            {footer?.socialLinks?.find((l: any) => l.platform?.toLowerCase() === 'linkedin')?.url && (
                            ...
                            {/* Twitter / X */}
                            {footer?.socialLinks?.find((l: any) => l.platform?.toLowerCase() === 'twitter')?.url && (
                            ...
                            {/* Instagram */}
                            {footer?.socialLinks?.find((l: any) => l.platform?.toLowerCase() === 'instagram')?.url && (
                            ...
                            {/* YouTube */}
                            {footer?.socialLinks?.find((l: any) => l.platform?.toLowerCase() === 'youtube')?.url && (
                    ...
                    <div>
                        ...
                        {(footer?.footerLinks?.length ?? 0) > 0 ? (
                            <ul className="space-y-3">
                                {footer?.footerLinks?.map((link: any, i: number) => (
                                    <li key={i}>
                                        <Link href={link.url} className="text-gray-500 hover:text-red-600 text-sm transition-colors duration-200 hover:translate-x-1 inline-block">
                                            {link.label}
                                        </Link>
                                    </li>
                                ))}
                        ...
                        <ul className="space-y-4">
                            ...
                            {contactInfo?.phone && (
                                <li className="flex items-center space-x-3">
                                    ...
                                    <span className="text-gray-500 text-sm">{contactInfo.phone}</span>
                                </li>
                            )}
                            {footer?.footerContent && (
                                <li className="flex items-start space-x-3 mt-4 pt-4 border-t border-gray-100">
                                    <span className="text-gray-500 text-sm whitespace-pre-wrap">{footer.footerContent}</span>
                                </li>
                            )}
                        </ul>
                    </div>
            ...
            <div className="relative z-10 border-t border-red-100/50">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col sm:flex-row justify-between items-center">
                    <p className="text-gray-400 text-xs">
                        {footer?.copyrightText || `© ${new Date().getFullYear()} Econiya Technologies. All rights reserved.`}
                    </p>
